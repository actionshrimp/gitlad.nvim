...
function M.open(status_buffer)
  ...
  local help_popup = popup
    ...
    :action("<S-Tab>", "Toggle all sections", function()
      status_buffer:_toggle_all_sections()
    end)
    :action("1", "Show headers only (scoped)", function()
      status_buffer:_apply_scoped_visibility_level(1)
    end)
    :action("2", "Show items (scoped)", function()
      status_buffer:_apply_scoped_visibility_level(2)
    end)
    :action("3", "Show diffs (scoped)", function()
      status_buffer:_apply_scoped_visibility_level(3)
    end)
    :action("4", "Show all (scoped)", function()
